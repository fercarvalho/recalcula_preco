...
interface GerenciamentoPlataformasDemoProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const GerenciamentoPlataformasDemo = ({ isOpen, onClose }: GerenciamentoPlataformasDemoProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const plataformas = sessionStorageService.obterPlataformas();
    ...
    if (modoMultiplo) {
      ...
      try {
        ...
        for (const plataforma of plataformasValidas) {
          const novaPlataforma = sessionStorageService.criarPlataforma(plataforma.nome, plataforma.taxa);
          novasPlataformas.push(novaPlataforma);
        }

        setPlataformas(novasPlataformas);
        sessionStorageService.salvarPlataformas(novasPlataformas);
      ...
      try {
        if (editingPlataforma) {
          const plataformaAtualizada = sessionStorageService.atualizarPlataforma(editingPlataforma.id, { nome: formNome.trim(), taxa });
          ...
          const novaPlataforma = sessionStorageService.criarPlataforma(formNome.trim(), taxa);
        ...
        await mostrarAlert('Erro', error.message || `Erro ao ${editingPlataforma ? 'atualizar' : 'adicionar'} plataforma.`);
      }
    }
  };
...
export default GerenciamentoPlataformasDemo;
