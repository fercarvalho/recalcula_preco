...
interface PainelAdminDemoProps {
  isOpen: boolean;
  onClose: () => void;
  scrollToColors?: boolean;
}

const PainelAdminDemo = ({ isOpen, onClose, scrollToColors = false }: PainelAdminDemoProps) => {
  const configInicial = sessionStorageService.obterConfiguracoes();
  const [config, setConfig] = useState<ConfiguracoesDemo>(configInicial);
  const [logoPreview, setLogoPreview] = useState<string | null>(configInicial.logoUrl || null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (isOpen) {
      const configAtual = sessionStorageService.obterConfiguracoes();
      setConfig(configAtual);
      setLogoPreview(configAtual.logoUrl || null);
      aplicarConfiguracoesDemo(configAtual);
    }
  }, [isOpen]);
  ...
  const handleCorChange = (campo: keyof ConfiguracoesDemo, valor: string) => {
    const novoConfig = { ...config, [campo]: valor };
    setConfig(novoConfig);
    aplicarConfiguracoesDemo(novoConfig);
  };
  ...
  const handleResetar = async () => {
    ...
    if (confirmado) {
      const configPadrao: ConfiguracoesDemo = {
        corPrimaria: '#FF6B35',
      ...
      setLogoPreview(null);
      aplicarConfiguracoesDemo(configPadrao);
      
      sessionStorageService.salvarConfiguracoes(configPadrao);
  ...
  const handleSalvar = () => {
    sessionStorageService.salvarConfiguracoes(config);
    aplicarConfiguracoesDemo(config);
...
export default PainelAdminDemo;
